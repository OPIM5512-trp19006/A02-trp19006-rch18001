...
# Plot actual versus predicted values for the tuned training set.
fig, ax = plt.subplots(figsize=(8, 6))
ax.scatter(y_train, y_train_tuned_pred, alpha=0.25, s=12)
plot_min = min(y_train.min(), y_train_tuned_pred.min())
plot_max = max(y_train.max(), y_train_tuned_pred.max())
ax.plot([plot_min, plot_max], [plot_min, plot_max], color="black", linestyle="--")
ax.set_xlabel("Actual MedHouseVal")
ax.set_ylabel("Predicted MedHouseVal")
ax.set_title("Tuned Training Set: Actual vs Predicted")
ax.grid(alpha=0.2)
fig.tight_layout()
fig.savefig(os.path.join(figs_dir, "train_tuned_actual_vs_pred.png"), dpi=300)
plt.close(fig)
...
# Plot actual versus predicted values for the tuned test set.
fig, ax = plt.subplots(figsize=(8, 6))
ax.scatter(y_test, y_tuned_pred, alpha=0.25, s=12)
plot_min = min(y_test.min(), y_tuned_pred.min())
plot_max = max(y_test.max(), y_tuned_pred.max())
ax.plot([plot_min, plot_max], [plot_min, plot_max], color="black", linestyle="--")
ax.set_xlabel("Actual MedHouseVal")
ax.set_ylabel("Predicted MedHouseVal")
ax.set_title("Tuned Test Set: Actual vs Predicted")
ax.grid(alpha=0.2)
fig.tight_layout()
fig.savefig(os.path.join(figs_dir, "test_tuned_actual_vs_pred.png"), dpi=300)
plt.close(fig)
